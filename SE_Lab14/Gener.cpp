...
namespace Gener
{
	...
	void Data(std::ofstream* stream, IT::IdTable& id)
	{
		*stream << "\n.data\n";
		...
		// ИЗМЕНЕНИЕ 1: Форматы вывода заменены на %u (unsigned)
			...
			*stream << "M" << i << ": ";
			...
			case CodeGen::CMD_MOD:
			{
				...
				*stream << "pop eax\n";
				...
				// ИЗМЕНЕНИЕ 2: Беззнаковое умножение
				else if (instr.op == CodeGen::CMD_MUL) {
					*stream << "mul ebx\n";
				}
				// ИЗМЕНЕНИЕ 3: Беззнаковое деление (убрали cdq, добавили mov edx, 0)
				else if (instr.op == CodeGen::CMD_DIV) {
					*stream << "mov edx, 0\n";
					*stream << "div ebx\n";
				}
				// ИЗМЕНЕНИЕ 4: Беззнаковый остаток
			...
			case CodeGen::CMD_CMPL:
			{
				...
				*stream << "mov eax, 0\n";
				...
				// ИЗМЕНЕНИЕ 5: Беззнаковые условия (Above/Below вместо Greater/Less)
