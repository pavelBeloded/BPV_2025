...
namespace Error {
	...
	// 110-119  - Ошибки открытия и чтения (Лексер, низкий уровень)
	// 200-299  - Ошибки таблиц (IT, LT)
	// 300-399  - Семантические ошибки (типы, области видимости)
	// 600-699  - Синтаксические ошибки (GRB)
	// 700-799  - Ошибки генерации кода и ПОЛИЗ
	...
	ERROR errors[ERROR_MAX_ENTRY] = {
		...
		ERROR_ENTRY(116, "Ошибка в записи литерала (например, цифра > 7 в восьмеричном)"), // Для 012
		...
		ERROR_ENTRY(203, "Неверная запись идентификатора (слишком длинный)"),
		ERROR_ENTRY(204, "Превышен максимальный размер таблицы идентификаторов"),
		ERROR_ENTRY(205, "Недопустимый размер таблицы идентификаторов"),
		ERROR_ENTRY(206, "Ошибка создания таблицы идентификаторов (память)"),
		ERROR_ENTRY(207, "Попытка добавить пустой идентификатор"),
		...
		ERROR_ENTRY(214, "Ошибка создания лексической таблицы (память)"),
		...
		// --- 300-399: СЕМАНТИКА (Самое важное сейчас) ---
		...
		// --- 700-799: ПОЛЬСКАЯ ЗАПИСЬ И ГЕНЕРАЦИЯ ---
		ERROR_ENTRY(700, "Ошибка преобразования в Польскую запись"),
